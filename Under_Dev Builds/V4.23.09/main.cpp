...
{   system("clear");
    ...
    layout();
    logo();gotoxy(32,10);
    cout<<red<<bold<<UNDERLINE<<"Operating Environment ver: 4.12.03"<<cc;
    string command;
    while(true)
    {
        ...
        {   gotoxy(32,11);
            ...
            {    gotoxy(35,14);
                 ...
                 gotoxy(35,15);cout<<blue<<bold<<"\tTicTacToe";//TicTacToe
                 gotoxy(35,16);cout<<red<<bold<<"\tCalculator";//Calculator
                 gotoxy(35,17);cout<<blue<<bold<<"\tQuiz";//Quiz
                 gotoxy(35,18);cout<<red<<bold<<"\tTemp_Conv";//Temperature_Converter
                 gotoxy(35,19);cout<<blue<<bold<<"\teqn_solve";//Pair of linear equations in two variables
                 gotoxy(35,20);cout<<red<<bold<<"\tquad_eqn_solve, to open the applications(if installed).";//Quadratic Equation SOlver
                 gotoxy(35,21);cout<<yellow<<bold<<"Type:"<<cc;//New Section
                 gotoxy(35,22);cout<<red<<bold<<"\texit: To exit .";//Exit COmmand
                 gotoxy(35,23);cout<<blue<<bold<<"\tver: To see version number";//Version Number Command
                 gotoxy(35,24);cout<<red<<bold<<"\tinst_all: To install al applications except TicTacToe and Quiz at once.";//Installer for programs
                 gotoxy(35,25);cout<<blue<<bold<<"\tsettings: To change your existing settings.";//Settings Application
                 gotoxy(35,26);cout<<red<<bold<<"\tType 'registryeditor' to enter the mode"; //Registry KEy modifier/ manipulator.
                 gotoxy(35,27);cout<<blue<<bold<<"\tTo re-login type login.";//Login
                 gotoxy(35,28);cout<<red<<bold<<"\tTo install an app from above  list type inst_'with app name'";//Install command for individual Application
                 gotoxy(35,29);cout<<blue<<bold<<"\tTo uninstall an app from above  list type unst_'with app name'";//Un-install command for individual Application
                getchar();getchar();
            ...
            else if(command=="inst_ar(t_c)")//Installer command for TicTacToe
            {
             ...
             else if(command=="Calculator"||command=="calculator")//COmmand for calculator
            {
                ...
                {    gotoxy(32,15);  cout<<red<<bold"  Status: Non installed yet"<<cc;//if not  installed
                cout<<"\a\a"<<flush;
                   getchar();
                    getchar();
                    prompt();
            ...
            {cout<<"\a\a\a\a\a\a\a"<<flush;reg_status=0;//here it starts without admin previleges or manipulating 
                reg_edit();
            }
            ...
            else if(command=="inst_all")//Installer for applications other than TicTacToe and Quiz
            {
                reg=1;
                 fstream tc,d,c,eqn,qe,at;
                  ...
                  qe<<reg;qe.close();
                   at.open("quad_solve.txt",ios::out);
                  at<<reg;
                  at.close();gotoxy(32,15);
...
{
gotoxy(1,1);layout();logo();gotoxy(10,8); string c;
 reg_status=1;//Added support for adming reg_edit
    cout<<BLUE<<bold<<"Do you want to directly install this application or you want to use it once before installation?(Enter choice below)."<<cc<<cc;//Same strings that are displayed there
     gotoxy(32,10);    cout<<purple<<bold<<UNDERLINE"Help Me Decide:"<<cc;
    gotoxy(32,12);    cout<<yellow<<bold<<"If you wish to install the program directly on the browser without testing for once press y.";
     gotoxy(32,13);    cout<<"If you want to test the progarm before installing i.e. if you want to have a look at the user ";
     ...
     gotoxy(32,18);    cout<<cyan<<bold<<"Enter your choice(y/n):";
  ...
  cout<<cc;//making everythinh again normal afetr taking the input
    ...
    {system("clear");    layout();    logo();        gotoxy(32,8);
	   cout<<UNDERLINE<<red<<bold<<"While the game is loading let us know its working:"<<CLOSEUNDERLINE;
	   gotoxy(32,10);
	   cout<<yellow<<"At first type help command at the prompt.";	   gotoxy(32,11);
	   cout<<"You will be getting a list of available commands use it  "<<CLOSEUNDERLINE;	   gotoxy(88,10);
	   cout<<purple<<"  Desingned for ease"<<cc;	   gotoxy(32,12);
	   cout<<yellow<<"to install your desired program"<<cc;	   gotoxy(88,11);
	   cout<<purple<<"  For ease in using "<<cc;	   gotoxy(32,13);
	   cout<<yellow<<"Enjoy using it!!!"<<cc;	   gotoxy(88,12);
	   cout<<purple<<"  softwares"<<cc;	   gotoxy(88,17);
	   cout<<green<<"DEVELOPER:"<<RED<<" Subhajit Halder";	   gotoxy(2,6);
	   progressbar();	   sleep(6);	   add_user();      reg=0;
...
{gotoxy(1,1);    layout();    gotoxy(5,7);
    ...
    gotoxy(5,8);    cout<<green<<bold<<"Available commands:"<<cc;	//Displaying version numbers and available commanda
    ...
    gotoxy(5,16);    cout<<"\th.)help (It is adviced to use help command first to gets uses of available commands."<<cc;;
    while(true)//Starting an infinite loop to take use input n number of times
    {string command;      gotoxy(5,17);      cout<<"Command->";
      cin>>command;      fstream t,q,tc,c,eqn,d;      reg=1;
            if(command=="install")
            {    if(reg_status==1)//
                {install();                  gotoxy(5,18);
                  cout<<cyan<<bold<<"Succesfully completed the command";
                  getchar(); getchar();                  reg_edit();
                }
                  else 
                  {  gotoxy(5,18);
                  cout<<cyan<<bold<<"Succesfully completed the command";
                  ...
                  d.open("Calc.txt",ios::out);         d<<reg;                  d.close();
            ...
            {gotoxy(5,20);                cout<<blue<<bold<<" Welcome to HELP Documentation Ver: 1.0. :"<<cc;
                gotoxy(5,21);                cout<<"\ta.)install: To install all the programs at once.";
                gotoxy(5,22);                cout<<"\tb.)add_key (Only for Quiz and TicTacToe): To install only TicTacToe and Quiz[with admin previleges only].";
                gotoxy(5,23);                cout<<"\tc.)exit: Exit the Registry Editor and open the login screen.";
                gotoxy(5,24);                cout<<"\td.)temp: To access a program temporaily(For TicTacToe and QUIZ).";
